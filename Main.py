...
def crear_modelo():
    ...
    model = tf.keras.Sequential([base_model,global_average_layer,keras.layers.BatchNormalization(),keras.layers.Dropout(rate=0.6),layers.Dense(len(set_labels), activation=keras.activations.hard_sigmoid,dtype=tf.float32)])
    return model
